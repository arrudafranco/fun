...
The desktop UI features switchable Overview/Detail modes for blocs, policies, milestones, and the sidebar, collapsible sections, dynamic resource bar coloring, a milestone tracker with condition checklists, an always-visible toolbar End Turn button with action count, close buttons on all informational modals, save/load with one-click saving and confirmation-guarded loading, and a guided 15-step tutorial with spotlight highlighting. Fully responsive for mobile with a 5-tab layout.
